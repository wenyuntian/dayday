...
const postData = allPost.map(p => {
  ...
  const tagHandler = /标签(:|：).+\n/gi.exec(article);
  const originHandler = /原著(:|：).+\n/gi.exec(article);
  const summaryHandler = /概述(:|：).+\n/gi.exec(article);
  ...
  const tag = tagHandler.length
    ...
    : '';
  const origin = originHandler.length
    ? originHandler[0].replace(/(原著(:|：)\s*|\n)/gi, '')
    : '';
  const summary = summaryHandler.length
    ? summaryHandler[0].replace(/(概述(:|：)\s*|\n)/gi, '')
    : '';
